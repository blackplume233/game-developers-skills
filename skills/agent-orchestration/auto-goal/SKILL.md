...
- 如果工作区或仓库明确指定 artifact 根目录，优先使用该目录。
- Auto Goal 的常态结构是 `<artifact-root>/goal/<date-slug>/`；也就是说 `/goal/...` 是挂在当前工作环境 artifact 根目录下的固定子结构。
- 如果仓库只指定 output、task-result 或 goal-result 目录，但没有指定 artifact 根目录，使用该目录下最接近 `goal/<date-slug>/` 的等价结构。
- 如果仓库没有定义产物目录，本工作环境默认 artifact 根目录为 `.artifact`，因此使用 `.artifact/goal/<date-slug>/`。
- 如果已有任务目录在 `.agents/artifact/goal/`，继续复用该目录，不要为了默认值迁移活跃任务。
...
使用 `assets/template/goal.md` 作为初始形状。有效的 `goal.md` 只包含顶部程序控制 frontmatter 和下面三个人工协作章节：

- 文件顶部 `---` YAML frontmatter：包含机器可读状态字段，格式与 `SKILL.md` 元数据一致。
- `目标`：详细、无歧义的基础目标和紧凑的完成证据。
- `约束`：范围限制、排除项、安全规则、兼容需求、仓库语言偏好和技术要求。
- `执行指导`：每轮如何拆分子目标、遵循仓库约定并验证。

程序控制字段必须放在文件顶部 YAML frontmatter 中，避免和正文说明混在一起。旧的 `## 状态` Markdown 表格或 YAML 代码块仍可读取和校验，但创建或实质更新时应迁移为顶部 frontmatter。必需状态键是 `status`、`updated_at`、`artifact_language`、`execution_mode`、`native_goal_id`、`current_loop`、`next_sub_goal`、`last_verified_loop` 和 `references`。
...
```bash
python .agents/skills/auto-goal/scripts/goal_state.py read <goal.md>
python .agents/skills/auto-goal/scripts/goal_state.py get <goal.md> status
python .agents/skills/auto-goal/scripts/goal_state.py set <goal.md> next_sub_goal "..." --in-place
python .agents/skills/auto-goal/scripts/goal_state.py migrate <goal.md> --in-place
```

脚本会把旧 `## 状态` 表格或 YAML 状态迁移为顶部 frontmatter。不要用临时正则或手工字符串拼接改状态字段。

## 目标确认和 Grill-Me 澄清

生成或大幅改写 `goal.md` 后，必须邀请用户确认，然后才能进入执行循环，除非用户明确要求先执行且目标已经无歧义、低风险。

确认流程：

1. 写出 `goal.md` 草案，状态设为 `draft`，`next_sub_goal` 写成“等待用户确认目标文档”。
2. 向用户给出简短确认请求，要求重点检查基础目标、完成证据、约束、排除项、语言和验证方式。
3. 用户确认后，把 `status` 改为 `active`，设置第一个可执行 `next_sub_goal`，再进入原生 Goal 桥接或执行循环。
4. 如果用户修改 `goal.md` 或聊天中改变目标，重新读取并必要时回到确认流程。

遇到表述不清、边界不明或证据不可验证的目标时，使用 `grill-me` 思路逐分支澄清：

- 一次只问一个关键问题。
- 如果问题可通过阅读代码、README、任务文件或现有约束回答，先自己查证，不要问用户。
- 每个问题给出推荐答案和原因，方便用户快速确认或修正。
- 沿决策树推进：目标范围 → 非目标 → 成功证据 → 验证方式 → 约束/风险 → 优先级。
- 不要把用户未确认的推断写成事实；可以写入 `references.md` 的“待确认假设”。
- 如果用户不在线且合理默认足够安全，记录假设并继续低风险准备工作；高风险或会改变目标含义时等待确认。
...
1. 先把用户请求转换为详细、无歧义的目标表述，再执行：
   ...
   - 识别假设和未知项。
   - 当歧义会导致执行不安全、大概率走错、完成证据不可验证，或目标范围可能被错误缩小/扩大时，按 `grill-me` 思路一次问一个问题。
...
6. 创建或实质修改后，运行 `scripts/validate_goal_file.py <goal.md>`。
7. 邀请用户确认 `goal.md`。确认前保持 `status: draft`，除非用户已明确授权跳过确认且目标无歧义、低风险。
8. 用户确认后，把状态更新为 `active`，再尝试桥接到宿主原生 Goal 模式；不可用时记录为文件模拟模式。
9. 每轮开始前必须重新读取 `goal.md`。用户可能在执行期间直接编辑它。
10. 如果循环会触及新模块、工具、语言、生成产物或外部系统，重新检查任务相关仓库基准文件。
11. 审查适用参考模式：
...
12. 根据基础目标和当前状态推导一个有边界的子目标。
13. 创建或更新 `loops/loop-XXX.md`，记录：
    - 正在处理的基础目标切片。
    - 子目标。
    - 本轮仓库基准检查。
    - 计划。
...
14. 如果子目标仍然太宽，按下面的 HTN 规则递归分解，不要假装它已经可执行。
15. 执行子目标；当子代理能提升隔离性、审查质量、探索能力或并行进度时，多使用子代理。
16. 验证子目标是否真正推进基础目标，并满足仓库基准。
17. 更新文件：
...
- 添加失败检查后的延续任务。
- 记录新发现的约束。
- 在循环结束时用 `goal_state.py` 更新顶部 frontmatter 状态。
