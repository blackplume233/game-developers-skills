...
- **黑盒为主，白盒为辅**：实际执行的操作必须是真实用户能通过界面、命令行或公开 API 自行复现的动作；DOM/日志/内部状态/数据库只用于事后定位原因，不能代替用户操作本身（例如直接改库、调用内部调试接口、跳过登录鉴权）。确需白盒手段时单独记录为诊断证据，不计入操作步骤。
- **方案先行**：正式操作真实入口前，先写出用户可复现的测试步骤和预期结果——`run`/`create`/`loop` 落地为场景文件，`explore` 至少先写一个最小 charter（目标、计划步骤、预期信号）；方案确认后才开始执行，不允许边测边想、执行完再补方案。
- **工程判断**：不迷信机械断言；综合截图、可见文本、日志、网络/API 响应、上下文连贯性判断 PASS/WARN/FAIL。
- **证据完整**：关键操作前后保留截图、日志、命令输出、请求响应摘要和复现步骤。
- **增量记录**：每一个执行的操作完成后立即写日志，不能遗漏任何一步，也不能把观察结果积攒到最后。
...
`action` 中 `screenshot` / `navigate` / `click` / `type` / `pressKey` / `command` / `shell` / `request` 必须对应真实用户能自己执行的操作；`evaluate` / `hasText` / `waitForText` / `queryText` 等观察类动作只用于读取和判断结果，不能替代上面的用户操作出现在步骤序列里冒充"操作步骤"。
...
2. 在操作真实入口前，先写一个最小 charter：本轮计划走的用户可复现步骤序列，以及每步预期结果（哪怕只有 2-3 步），记录到当轮日志开头；charter 可以随探索增量补充新步骤，但已执行步骤的预期不能事后改写。
3. 操作真实入口，按 charter 记录每个观察点和证据。
4. 对异常行为给出 PASS/WARN/FAIL 判断和理由。
5. 结束时必须输出「新测试用例候选」，说明是否建议保存。
6. 用户提供 `--save-case` 或发现关键缺陷时，保存场景到项目内。
...
8. **操作用户可复现**：黑盒步骤中实际执行的动作必须是真实用户能照做的操作，不用内部旁路、直接改库或跳过鉴权代替；确需白盒手段定位原因时，单独记录为诊断证据，不计入用户操作步骤。
9. **先出方案再执行**：`run`/`create`/`loop` 必须先有场景文件（步骤 + 预期），`explore` 至少先写最小 charter；禁止边测边设计或执行完再补方案。
